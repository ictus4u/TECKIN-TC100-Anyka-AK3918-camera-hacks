...
rtsp_h26x_server() {
	case "${1}" in
		...
		off)
			...
			;;
		status)
			if /mnt/controlscripts/rtsp-h26x status | grep -q "PID"; then
				echo "ON"
			else
				echo "OFF"
			fi
...
night_mode() {
	case "${1}" in
		...
		off)
			...
			;;
		status)
			if /mnt/controlscripts/night-mode status | grep -q "PID";	then
				echo "ON"
			else
				echo "OFF"
			fi
...
auto_night_mode() {
	case "${1}" in
		...
		off)
			...
			;;
		status)
			if /mnt/controlscripts/auto-night-detection status | grep -q "PID"; then
				echo "ON"
			else
				echo "OFF"
			fi
